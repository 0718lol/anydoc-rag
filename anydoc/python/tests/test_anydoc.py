...
class AnydocTest(unittest.TestCase):
    ...
    def test_to_rag_returns_markdown_chunks_and_asset_metadata(self):
        payload = anydoc.to_rag(RICH, max_chars=400, overlap=0)
        self.assertIn("| Quarter | Widgets |", payload["markdown"])
        self.assertEqual(payload["metadata"]["source"], str(RICH))
        self.assertEqual(payload["metadata"]["format"], "docx")
        self.assertEqual(payload["metadata"]["chunk_count"], len(payload["chunks"]))
        self.assertEqual(payload["metadata"]["asset_count"], len(payload["assets"]))
        self.assertRegex(payload["metadata"]["sha256"], r"^[0-9a-f]{64}$")
        self.assertGreater(len(payload["chunks"]), 0)
        self.assertTrue(all(chunk["id"].startswith("chunk-") for chunk in payload["chunks"]))
        self.assertTrue(any(chunk["metadata"]["headings"] for chunk in payload["chunks"]))

        image = next(asset for asset in payload["assets"] if asset["media_type"] == "image/png")
        self.assertEqual(set(image), {"id", "media_type", "origin_part", "bytes"})
        self.assertGreater(image["bytes"], 0)

    def test_to_rag_bytes_accepts_named_csv_and_validates_chunk_options(self):
        payload = anydoc.to_rag_bytes(CSV.read_bytes(), "csv", source="sheet.csv")
        self.assertEqual(payload["metadata"]["source"], "sheet.csv")
        self.assertEqual(payload["metadata"]["format"], "csv")
        ...
        with self.assertRaisesRegex(ValueError, "max_chars"):
            anydoc.to_rag_bytes(CSV.read_bytes(), "csv", max_chars=100)
        with self.assertRaisesRegex(ValueError, "overlap"):
            anydoc.to_rag_bytes(CSV.read_bytes(), "csv", overlap=-1)
        with self.assertRaisesRegex(ValueError, "overlap"):
            anydoc.to_rag_bytes(CSV.read_bytes(), "csv", max_chars=300, overlap=300)
    ...
    def test_ocr_local_reads_markdown_from_stdout_and_only_runs_when_needed(self):
        command = [sys.executable, "-c", "print('# Read locally')", "{input}"]
        self.assertEqual(
            anydoc.to_markdown_bytes(MIXED.read_bytes(), ocr="local", ocr_command=command),
            "# Read locally\n",
        )
        self.assertRegex(
            anydoc.to_markdown(OUTLINE, ocr="local", ocr_command=["no-such-command"]),
            r"(?m)^# ",
        )

    def test_ocr_local_can_read_an_output_file_and_reports_failures(self):
        script = "from pathlib import Path; Path(__import__('sys').argv[1]).write_text('# File output')"
        command = [sys.executable, "-c", script, "{output}", "{input}"]
        self.assertEqual(
            anydoc.to_markdown(MIXED, ocr="local", ocr_command=command), "# File output\n"
        )
        with self.assertRaisesRegex(anydoc.LocalOcrError, "not configured"):
            anydoc.to_markdown_bytes(MIXED.read_bytes(), ocr="local")
        with self.assertRaisesRegex(anydoc.LocalOcrError, "status 7"):
            anydoc.to_markdown_bytes(
                MIXED.read_bytes(),
                ocr="local",
                ocr_command=[sys.executable, "-c", "raise SystemExit(7)", "{input}"],
            )
    ...
    def test_unreadable_files_and_bad_arguments_raise_the_python_exception(self):
        ...
        with self.assertRaisesRegex(ValueError, "unknown OCR mode"):
            anydoc.to_markdown(OUTLINE, ocr="wat")

    def test_ocr_paddleocr_collects_markdown_from_the_structure_output_directory(self):
        script = "from pathlib import Path; import sys; Path(sys.argv[1]).mkdir(exist_ok=True); Path(sys.argv[1], 'result.md').write_text('# Paddle')"
        command = [sys.executable, "-c", script, "{output_dir}", "{input}"]
        self.assertEqual(
            anydoc.to_markdown(MIXED, ocr="paddleocr", paddleocr_command=command),
            "# Paddle\n",
        )
    ...
    def test_the_stubs_cover_the_module(self):
        ...
        self.assertEqual(
            set(anydoc.__all__),
            exported
            | {"Format", "HostedError", "LocalOcrError", "Ocr", "to_rag", "to_rag_bytes"},
        )
