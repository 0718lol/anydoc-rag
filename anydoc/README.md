...
# Or keep OCR local through a command that returns Markdown:
...
# Or use PaddleOCR locally (Python):
markdown = anydoc.to_markdown("scan.pdf", ocr="paddleocr")
...
anydoc reads text-based PDFs locally but does no OCR, so a PDF with scanned or image-only pages fails with `NeedsOcr`. Opt in to Firecrawl Parse, or use the Python binding's local command adapter to keep the document on your machine.
...
| Python (local) | `anydoc.to_markdown("scan.pdf", ocr="local", ocr_command=[...])` | `ocr_command`, `ocr_timeout` |
| Python (PaddleOCR) | `anydoc.to_markdown("scan.pdf", ocr="paddleocr")` | `paddleocr`, `paddlepaddle` |

Only documents using hosted OCR leave the machine, and the whole document goes, since Parse has no page selection. Local Python OCR commands receive `{input}` and can write Markdown to `{output}` or stdout; the PaddleOCR adapter collects Markdown from its output directory. If Parse cannot convert a document, Node rejects with `code: 'hosted'` and Python raises `HostedError`. `--api-url`, `apiUrl` and `api_url`, else `FIRECRAWL_API_URL`, point at another Parse deployment. The Rust crate has no `ocr` option and never makes network calls.
